...
Next, ask the user which issues they want to fix by either replying "all", or a comma separated list of IDs. After their reply, run a separate sub agents (#runSubAgents) to fix each issue that the user has specified. each sub agent should report back back with a simple `subAgentSuccess : true | false`.